...
class archiveDbModelArticles  extends JModelLegacy
{
    ...
    $sql = "SELECT 	a.article_id, `issue_id`, `art_type`, `pdf`, `pages`, `published`, `translation`, `udk`, `doi`, `submited_date`, `edited_date`, `published_date`, `create_date`, `edit_date`
    ...
    foreach ($this->_db->loadObjectList() as $item) {
			...
			$sql = "SELECT 	`section`, `title`, `abstract`, `language`
    ...
    $articles = &$issue->articles;
		...
		foreach ($articles as &$article) {
          ...
          // если статья редактируется
            ...
            $sql .= "INSERT INTO #__sjarchive_meta (
              `article_id`,
              `language`,
              `title`,
              `abstract`,
              `section`)
            VALUES (
              {$this->_db->quote($article->ID)},
              {$this->_db->quote($language)},
              {$this->_db->quote($article->title[$language])},
              {$this->_db->quote($article->abstract[$language])},
              {$this->_db->quote($article->section[$language])})
            ON DUPLICATE KEY UPDATE `article_id` = {$this->_db->quote($article->ID)},
              `language` = {$this->_db->quote($language)},
              `title` = {$this->_db->quote($article->title[$language])},
              `abstract` = {$this->_db->quote($article->abstract[$language])},
              `section` = {$this->_db->quote($article->section[$language])};
            ";
          }
          


					if ($article->keywords[$language] && !empty($article->keywords[$language])) {
                ...
                VALUES (@article_id,LAST_INSERT_ID(),{$this->_db->quote($keyword->language)});";
					...
					if ($article->reference[$language] && !empty($article->reference[$language])) {
            ...
            if (!$author[$language]->authorId) {
              ...
              $sql .= "INSERT INTO #__sjarchive_author ( 
                      ...
                      ON DUPLICATE KEY UPDATE 
                        ...
                        `other` = {$this->_db->quote($author[$language]->other)}; ";
