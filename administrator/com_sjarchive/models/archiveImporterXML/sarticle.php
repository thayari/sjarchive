...
class archiveImporterXMLSarticle
{
	const SCHEMA = NULL;
	protected $xml;
	...
	public function importIssue($doi_settings = array('core' => '10.22227', 'issn' => '2073-8412', 'useNum' => '1', 'x' => 0, 'useVolume' => 0, 'useYear' => 1, 'useDoi' => 1))
	{
		...
		$this->_item['issue']->num				= (int) $this->xml->journal->issue->jnumUni;
		$this->_item['issue']->part	 			= (int) 0;
		$this->_item['issue']->volume 			= (int) 0;
		$this->_item['issue']->year 			= (int) $this->xml->journal->issue->jdateUni;
		$this->_item['issue']->pubDate  		= 0;
	...
	public function importArticles($languages, &$issue, $articles_settings = array('auto_publisher' => 1))
	{
		...
		foreach ($this->xml->xpath('journal/issue/article') as $no => $node) {
			...
			foreach ($languages as $language_tag) 
			{
				...
				$article->udk		= (string) $node->udk;
				$article->pages		= implode('-',array(trim((string) $node->fpageart),trim((string) $node->lpageart)));
				$article->pdf		= NULL;
				$article->hits		= 0;
				...
				$article->section[$language_tag]	= (string) array_shift($node->xpath("self::*/preceding-sibling::*[1]/segtitle[@lang='{$language}']"));
				...
				if ($language == 'RUS') {
					...
					foreach ($node->xpath('biblist/blistpart') as $item) 
					{
				...
				foreach ($node->xpath("keywords/kwdGroup[@lang='{$language}']/keyword") as $item) 
				{
				...
				foreach ($node->xpath('authors/author') as $node2) {
					...
					$author->other	= (string) array_shift($node2->xpath("individInfo[@lang='{$language}']/auinf"));
